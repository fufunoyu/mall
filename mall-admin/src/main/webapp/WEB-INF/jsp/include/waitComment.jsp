...
<body>
<%--轮播图信息列表--%>
<table id="waitComment_grid" class="easyui-datagrid" title="退货中订单列表"
...
<!-- 分页栏 -->
<div id="waitComment_pagination" class="easyui-pagination">
</div>
<script>
     ...
     * 加载数据网格
     */
    function waitCommentGrid() {
        $.ajax({
            url: '${pageContext.request.contextPath}/order/waitCommentList.json',
            method: 'get',
            success: function (data) {
//                console.log(data)
                waitCommentRefreshPage(data, 1)
            }
        })
    }
    ...
    function getShipIngIds() {
        var sels = $("#waitComment_grid").datagrid("getSelections");
        var ids = [];
        for (var i in sels) {
    ...
    <%--分页功能实现--%>
    $('#waitComment_pagination').pagination({
        onSelectPage: function (pageNumber, pageSize) {
            $(this).pagination('loading');
            // alert('pageNumber:' + pageNumber + ',pageSize:' + pageSize);
            $.ajax({
                url: '${pageContext.request.contextPath}/order/waitCommentList.json?page=' + pageNumber + "&size=" + pageSize,
                method: 'get',
                success: function (data) {
                    waitCommentRefreshPage(data, pageNumber)
                }
            })
     ...
     * 刷新列表
     */
    function waitCommentRefreshPage(data, pageNumber) {
        var arr = []
...
//                console.log(arr)
        $('#waitComment_grid').datagrid('loadData', arr)
        waitComentChangePage(pageNumber, data.count)
    }

    /**
     * 分页设置每页显示条数和当前目录及其子目录共有多少件商品
     * */
    function waitComentChangePage(page, total) {
        $('#waitComment_pagination').pagination({
            total: total,
            pageNumber: page
        });
